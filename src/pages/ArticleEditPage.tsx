...
import {
  ...
  Image, Upload, X, Tag, Link2, Plus, Paperclip
} from 'lucide-react'
...
function CoverImageUploader({
  ...
  const handleFile = async (file: File) => {
    setUploading(true)
    try {
      const asset = await assetService.uploadFile(worldId, file)
      ...
      {value ? (
        <div className="relative rounded-xl overflow-hidden border border-surface-500 mb-2">
          ...
          <button
            type="button"
            onClick={() => onChange('')}
            className="absolute top-2 right-2 p-1 bg-black/60 rounded-lg text-white hover:bg-red-600 transition-colors">
            <X size={14} />
          </button>
        </div>
      ) : (
        <div
          ...
          className="border-2 border-dashed border-surface-500 rounded-xl p-8 text-center cursor-pointer hover:border-brand-500 transition-colors mb-2">
          {uploading ? (
            <p className="text-slate-400 text-sm">Wird hochgeladen…</p>
          ) : (
            <>
              <Image size={28} className="mx-auto mb-2 text-slate-500" />
              <p className="text-slate-400 text-sm">Klicken zum Hochladen</p>
...
function HandoutUploader({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const asset = await assetService.uploadFile(worldId, file)
      onChange([...handouts, { name: file.name, url: asset.publicUrl, path: asset.path }])
    } catch {
      toast.error('Upload fehlgeschlagen')
    } finally {
      setUploading(false)
    ...
    <div>
      ...
      <div className="space-y-2">
        {handouts.map((h, i) => (
          <div key={i} className="flex items-center gap-3 card px-3 py-2">
            ...
            <a href={h.url} target="_blank" rel="noreferrer"
              className="flex-1 text-sm text-brand-400 hover:underline truncate">{h.name}</a>
            <button type="button" onClick={() => onChange(handouts.filter((_, idx) => idx !== i))}
        ...
        <button
          ...
          onClick={() => fileRef.current?.click()}
          disabled={uploading}
          className="btn-secondary text-sm py-1.5 w-full justify-center">
...
function TagManager({
  ...
  const { data: allTags } = useQuery({
    queryKey: ['tags', worldId],
    queryFn: () => articleService.listTags(worldId),
  })

  const { data: currentTags, refetch } = useQuery({
    queryKey: ['article-tags', articleId],
    queryFn: () => articleId ? articleService.getTagsForArticle(articleId) : Promise.resolve([]),
    enabled: !!articleId,
  })
  ...
  const toggleTag = async (tagId: string) => {
    ...
    const next = currentTagIds.includes(tagId)
      ...
      : [...currentTagIds, tagId]
    await articleService.setTagsForArticle(articleId, next)
    refetch()
  }
  ...
  const createAndAdd = async () => {
    ...
    qc.invalidateQueries({ queryKey: ['tags', worldId] })
    setNewTag('')
    if (articleId) {
      await articleService.setTagsForArticle(articleId, [...currentTagIds, tag.id])
      refetch()
    }
  }
    ...
    <div>
          ...
          return (
            <button
              ...
              className={`badge cursor-pointer transition-all ${
                active ? 'bg-brand-600 text-white' : 'bg-surface-600 text-slate-400 hover:bg-surface-500'
              }`}
              style={active && tag.color ? { backgroundColor: tag.color + 'cc' } : {}}>
              {tag.name}
            </button>
          )
        })}
      ...
      <div className="flex gap-2">
        <input value={newTag} onChange={e => setNewTag(e.target.value)}
          ...
          className="input text-sm py-1.5 flex-1" placeholder="Neuer Tag…" />
        <button type="button" onClick={createAndAdd} className="btn-secondary py-1.5 px-3">
          <Plus size={14} />
        </button>
      </div>
      {!articleId && (
        <p className="text-xs text-slate-500 mt-1">Tags können nach dem ersten Speichern vergeben werden.</p>
      )}
    </div>
  )
}
...
function BacklinksPanel({ articleId }: { articleId: string }) {
  const { data: backlinks } = useQuery({
    queryKey: ['backlinks', articleId],
    queryFn: () => articleService.getBacklinks(articleId),
    enabled: !!articleId,
  })

  if (!backlinks || backlinks.length === 0) {
    return <p className="text-sm text-slate-500 italic">Noch keine Backlinks</p>
  }
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {backlinks.map(bl => {
        ...
        return source ? (
          <a key={bl.source_article_id} href={`#/articles/${source.slug}`}
...
function GalleryManager({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      const asset = await assetService.uploadFile(worldId, file)
      onChange([...images, asset.publicUrl])
    } catch {
      toast.error('Upload fehlgeschlagen')
    } finally {
      setUploading(false)
          ...
          {images.map((url, i) => (
            <div key={i} className="relative rounded-lg overflow-hidden border border-surface-500 aspect-square">
              ...
              <button type="button" onClick={() => onChange(images.filter((_, idx) => idx !== i))}
                className="absolute top-1 right-1 p-0.5 bg-black/60 rounded text-white hover:bg-red-600 transition-colors">
                <X size={12} />
              </button>
            </div>
          ))}
...
function CustomFields({
  ...
  return (
    <div>
        ...
        {fields.map((f, i) => (
          <div key={i} className="flex gap-2 items-center">
            <input
              ...
              className="input text-sm py-1.5 w-1/3"
      ...
      <button type="button" onClick={() => onChange([...fields, { key: '', value: '' }])}
        className="btn-ghost text-sm py-1 text-slate-400">
...
export default function ArticleEditPage({ worldId }: { worldId: string }) {
  ...
  // Extended fields (stored in content_json meta or separate JSONB field)
  const [showAdvanced, setShowAdvanced] = useState(false)
  const [coverImage, setCoverImage] = useState('')
  const [gallery, setGallery] = useState<string[]>([])
  const [handouts, setHandouts] = useState<Handout[]>([])
  const [customFields, setCustomFields] = useState<CustomField[]>([])
  const [aliases, setAliases] = useState('')   // comma-separated alternative names
  const [lore, setLore] = useState('')          // GM-only notes / lore
  const [articleLinks, setArticleLinks] = useState<string[]>([]) // external URL links
  ...
  // Merge extended meta into content_json before saving
  const buildPayloadContentJson = () => {
    ...
    return {
      ...base,
      __meta: { coverImage, gallery, handouts, customFields, aliases, lore, articleLinks },
    }
  }
          ...
          <div className="card p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              <textarea value={summary} onChange={e => { setSummary(e.target.value); setSaved(false) }}
                className="input resize-none h-16" placeholder="Kurze Zusammenfassung…" />
            </div>
          </div>
            ...
            <button
              ...
              <span className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-500 font-normal">
                  – Titelbild, Galerie, Handouts, Tags, eigene Felder…
                </span>
              </span>
            ...
            <div className="space-y-6">
              ...
              <div className="card p-4">
                <CoverImageUploader
                  worldId={worldId}
                  value={coverImage}
                  onChange={v => { setCoverImage(v); setSaved(false) }}
                />
              </div>
              ...
              <div className="card p-4">
                <GalleryManager
                  worldId={worldId}
                  images={gallery}
                  onChange={v => { setGallery(v); setSaved(false) }}
                />
              </div>
              ...
              <div className="card p-4">
                <HandoutUploader
                  worldId={worldId}
                  handouts={handouts}
                  onChange={v => { setHandouts(v); setSaved(false) }}
                />
              </div>

              {/* Tags */}
              <div className="card p-4">
                <TagManager worldId={worldId} articleId={isNew ? undefined : article?.id} />
              </div>
              ...
              <div className="card p-4">
                ...
                <input
                  value={aliases}
                  onChange={e => { setAliases(e.target.value); setSaved(false) }}
                  className="input"
                  placeholder="Kommagetrennte Namen, z.B. Andergast, Die Grenzfestung, Fort Andergast"
                />
              ...
              <div className="card p-4">
                  ...
                  {articleLinks.map((link, i) => (
                    <div key={i} className="flex gap-2">
                      <input
                        value={link}
                        onChange={e => {
                          const next = [...articleLinks]
                          next[i] = e.target.value
                          setArticleLinks(next)
                          setSaved(false)
                        }}
                        className="input text-sm py-1.5 flex-1"
                        placeholder="https://…"
                      />
                      <button type="button" onClick={() => setArticleLinks(articleLinks.filter((_, idx) => idx !== i))}
                ...
                <button type="button"
                  onClick={() => { setArticleLinks([...articleLinks, '']); setSaved(false) }}
                  className="btn-ghost text-sm py-1 text-slate-400">
              ...
              <div className="card p-4 border border-amber-800/40 bg-amber-900/5">
                <label className="label text-amber-400 flex items-center gap-1">
                  <EyeOff size={12} /> GM-Notizen (intern, nie für Spieler sichtbar)
                </label>
                <textarea
                  value={lore}
                  onChange={e => { setLore(e.target.value); setSaved(false) }}
                  className="input resize-none h-28 bg-amber-900/10 border-amber-800/40"
                  placeholder="Geheime Hintergrundinformationen, Plottwists, Motivationen…"
                />
              </div>
              ...
              <div className="card p-4">
                <CustomFields
                  fields={customFields}
                  onChange={v => { setCustomFields(v); setSaved(false) }}
                />
              </div>
